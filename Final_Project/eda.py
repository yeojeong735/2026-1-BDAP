...
# --- 추가: 가성비 지표(Value_Score) 생성 ---
# RAM(GB)과 Battery Capacity(mAh)를 활용해 value score 계산
df_clean['Value_Score'] = (df_clean['RAM'] * 1000 + df_clean['Battery Capacity']) / (df_clean['Launched Price (USA)'] + 1)

# 상위 5개 행으로 특성 정상 추가 여부 확인
print("\n=== Value_Score 포함 상위 5개 데이터 ===")
print(df_clean[['Company Name', 'Model Name', 'RAM', 'Battery Capacity', 'Launched Price (USA)', 'Value_Score']].head(5))

# 5. 전처리된 데이터를 최종 업데이트하여 저장 (한글 깨짐 방지)
