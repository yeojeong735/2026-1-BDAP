...
# 1. 파일 경로 지정
file_path = r"C:\Users\6-112\Desktop\빅데이터분석프로그래밍\20241513_장여정\one1\Personal_Project\Mobiles_Dataset(2025).csv"
...
# 4. 전처리 결과 확인
print("\n=== 전처리 후 데이터 타입 정보 ===")
print(df[['Mobile Weight', 'RAM', 'Battery Capacity', 'Screen Size', 'Launched Price (USA)', 'Front Camera']].info())

print("\n=== 전처리 후 데이터 샘플 ===")
print(df[['Company Name', 'Model Name', 'RAM', 'Battery Capacity', 'Launched Price (USA)']].head(3))

print(len(df))
# 5. 전처리된 데이터를 새로운 파일로 저장 (Streamlit에서 안전하게 불러올 수 있도록 utf-8-sig 지정)
df.to_csv("cleaned_mobiles_2025.csv", index=False, encoding='utf-8-sig')
